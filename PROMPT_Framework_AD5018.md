...
├─────────────────────┼────────────────────────────────────────────┤
│ Tech & Cost Overview│ Ver Sección 2 del canvas (temperatura)    │
├─────────────────────┼────────────────────────────────────────────┤
│ Stack tecnológico   │ Se define en Fase M1 — no en este canvas  │
├─────────────────────┼────────────────────────────────────────────┤
...
## Pregunta central
> *¿Qué tecnología es la correcta para este problema, y cómo construimos el MVP de forma iterativa y documentada dentro del tiempo disponible?*

## Propósito de la fase
Esta fase tiene dos momentos distintos y secuenciales. Primero el equipo **decide** con qué tecnología construirá — y lo justifica. Luego **construye** de forma iterativa. Saltarse M1 para ir directo a M2 es el error más común: el equipo que empieza a construir sin haber elegido el stack correcto pierde semanas cambiando de herramienta a mitad del camino.

---

## M1 — Selección del Stack Tecnológico

### Pregunta central de M1
> *¿Cuál es la tecnología correcta para este problema específico, y podemos justificar la elección con criterios objetivos?*

### Principio fundamental
El curso ofrece un menú de referencia de herramientas — **no una lista obligatoria**. El equipo puede y debe usar cualquier tecnología que justifique adecuadamente. Una herramienta no listada en el curso es completamente válida si el argumento es sólido. Lo que se evalúa es el **razonamiento detrás de la elección**, no la herramienta en sí.

> **La pregunta que el equipo debe responder:** ¿Por qué esta herramienta y no las otras disponibles para el mismo problema?

### Criterios de selección del stack

El equipo evalúa cada herramienta candidata contra estos 5 criterios:

| Criterio | Pregunta clave |
|---|---|
| **Adecuación al problema** | ¿Esta herramienta resuelve el tipo de problema definido en la Fase P? |
| **Viabilidad técnica** | ¿El equipo puede aprender a usarla en el tiempo disponible? |
| **Costo** | ¿El costo es sostenible para un MVP de curso? *(ver Tech & Cost Overview de Fase O)* |
| **Escalabilidad** | Si el producto crece, ¿la herramienta puede crecer con él? |
| **Integración** | ¿Se conecta con las otras herramientas del stack sin fricción excesiva? |

### Decisión documentada

El equipo documenta su elección en la forma:

```
Herramienta elegida: _______________
Tipo de IA que implementa: _______________
Alternativas consideradas y descartadas:
  - [Alternativa 1] — descartada porque: _______________
  - [Alternativa 2] — descartada porque: _______________
Justificación final de la elección (3-5 líneas):
```

> **Esta decisión se documenta en la Plantilla 3 (AI Product Canvas), Sección 6 — Diseño específico por tipo de IA.** El Tech & Cost Overview de la Sección 2 es la exploración previa que informó esta decisión — no la decisión en sí.

---

### Menú de referencia del curso *(opciones, no obligaciones)*

Las siguientes herramientas son punto de partida — el equipo puede proponer alternativas justificadas.

#### Herramientas Primarias

**Teachable Machine** *(Google)*
Tipo: ML visual no-code | teachablemachine.withgoogle.com
Ideal para: clasificación de imágenes, audio y poses sin código
Limitación clave: solo clasificación — no regresión ni clustering

**n8n** *(n8n GmbH)*
Tipo: Orquestación de flujos con IA | n8n.io
Ideal para: agentes de IA, automatización, conexión entre herramientas
Nodos clave: AI Agent, HTTP Request, OpenAI/Anthropic, Webhook, Google Sheets

**Claude** *(Anthropic)*
Tipo: LLM | claude.ai / api.anthropic.com
Ideal para: system prompts complejos, razonamiento estructurado, análisis de documentos
Formas de uso: interfaz web (prototipado), API (integración en n8n), Claude Code (desarrollo asistido)

#### Herramientas Complementarias

| Herramienta | Tipo | Cuándo considerarla |
|---|---|---|
| **OpenAI GPT-4o** | LLM | Integración nativa en n8n, proyectos multimodales |
| **Google Gemini** | LLM | Integración con Google Workspace, uso gratuito vía AI Studio |
| **BigML** | ML visual tabular | Predicción y clustering con datos en CSV o Excel |
| **Voiceflow** | Chatbot no-code | Flujos conversacionales con control visual de diálogo |
| **Bubble** | Frontend no-code | Interfaz web propia conectada a APIs de IA |
| **Streamlit** | Frontend Python | Interfaces para modelos ML con Python básico |
| **Airtable / Google Sheets** | Datos simple | Persistencia de logs, conversaciones o predicciones |

#### Tabla de referencia rápida por tipo de proyecto

| Tipo de proyecto | Stack de referencia | Dificultad |
|---|---|---|
...
| Asistente conversacional con base de conocimiento | Voiceflow + OpenAI / Claude | 🟢 Baja |
...
> **Nota:** Esta tabla es orientativa. El equipo que propone un stack diferente con buena justificación obtiene mayor valoración que el que copia el stack sugerido sin argumentar.

---

## M2 — Construcción Iterativa del MVP

### Pregunta central de M2
> *¿Cómo construimos el MVP de forma iterativa, documentada y dentro del tiempo disponible?*

### Ciclo de construcción recomendado

```
Semana 7  →  M1 finalizado + configuración inicial del stack
Semana 8  →  Primer prototipo funcional + prueba interna del equipo
Semana 9  →  Segunda iteración + primera prueba con usuario externo
Semana 10 →  Tercera iteración + documentación de decisiones técnicas
Semana 11 →  MVP finalizado + preparación para evaluación (Fases P2 y T)
```

### Principios de construcción para equipos de administración

**1. M1 antes que M2 — siempre**
No se empieza a construir sin tener el stack justificado. Un cambio de herramienta en Semana 9 es una crisis — no una iteración.

**2. Empieza por el flujo más simple posible**
El primer prototipo debe hacer una sola cosa bien. Agrega complejidad solo después de que lo básico funcione con un usuario real.

**3. Documenta cada decisión técnica**
¿Por qué ese prompt? ¿Por qué esa fuente de datos? ¿Por qué ese nodo? La sustentación evalúa el razonamiento, no solo el resultado.

**4. Prueba con usuarios desde la semana 9**
Un usuario externo al equipo revela problemas que el equipo ya no puede ver. Mínimo 3 usuarios externos antes de la sustentación.

**5. El error documentado vale más que el éxito silencioso**
Lo que salió mal y cómo se resolvió es tan valioso en la sustentación como el MVP final. Los evaluadores distinguen entre un equipo que tropezó y aprendió, y uno que ocultó problemas.
...
## Error crítico de esta fase
> Saltarse M1 y empezar a construir sin haber justificado el stack.
> El equipo que elige la herramienta por familiaridad o por ser la primera que encontró — sin evaluar alternativas — descubre en Semana 10 que necesita cambiar de tecnología. A esa altura, ya no hay tiempo.
...
*Diseñado para AD5018 Inteligencia Artificial para Negocios*
